...
#else
static int _mkind_errno = 0;
int _makeindex_main(int argc, char *argv[]);
char* replace_extension(const char* aux_file, const char* new_ext) {
    if (aux_file == NULL || new_ext == NULL) {
        return NULL;
    }
    const char* dot = strrchr(aux_file, '.');
    size_t base_len;
    if (dot != NULL && strchr(dot, '/') == NULL && strchr(dot, '\\') == NULL) {
        base_len = dot - aux_file;
    } else {
        base_len = strlen(aux_file);
    }
    size_t new_len = base_len + strlen(new_ext) + 1; // +1 for null terminator
...
#endif
{
	...
	if (_mkind_errno != 0) return _mkind_errno;
#endif
		    open_sty(*++argv);
#ifdef WEBASSEMBLY_BUILD
	if (_mkind_errno != 0) return _mkind_errno;
