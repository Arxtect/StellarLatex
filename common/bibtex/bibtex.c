...
#define HISTORY_SPOTLESS 0
#define HISTORY_WARNING_ISSUED 1
#define HISTORY_ERROR_ISSUED 2
#define HISTORY_FATAL_ERROR 3
...
#endif
#define rust_output_handle_t FILE*
#define ttstub_input_close fclose
#define ttstub_output_close fclose
#define ttstub_output_putc(A, B) fputc(B, A)
#define ttstub_output_write(A, B, C) fwrite(B, 1, C, A)
#define ttstub_input_getc fgetc
#define ttstub_output_open(A, B) fopen(A, "w")
...
#define BIB_XRETALLOC(array_name, array_var, type, size_var, new_size)         \
  do {                                                                         \
    BIB_XRETALLOC_NOSET(array_name, array_var, type, size_var, new_size);      \
    size_var = new_size;                                                       \
  } while (0)

#define BIB_XRETALLOC_STRING(array_name, array_var, length, size_var,          \
                             new_size)                                         \
  (array_var) = (ASCII_code *)xrealloc(                                        \
      (array_var), (new_size) * (length + 1) * sizeof(ASCII_code))
...
static peekable_input_t *peekable_open(const char *path,
                                       int format) {
  rust_input_handle_t handle;
  peekable_input_t *peekable;

  char* real_path = kpse_find_file(path, format, 0);
  if (real_path == NULL) {
  	return NULL;
  }
...
static int peekable_close(peekable_input_t *peekable) {
  int rv;
...
static int peekable_getc(peekable_input_t *peekable) {
  int rv;
...
static void peekable_ungetc(peekable_input_t *peekable, int c) {
  /* TODO: assert c != EOF */
...
static bool eof(peekable_input_t *peekable) {
...
static bool eoln(peekable_input_t *peekable) {
  int c;
...
PRINTF_FUNC(1, 2) static void printf_log(const char *fmt, ...) {
  va_list ap;
...
static void mark_fatal(void) { history = HISTORY_FATAL_ERROR; }
...
static void print_confusion(void) {
...
static void buffer_overflow(void) {
  BIB_XRETALLOC_NOSET("buffer", buffer, ASCII_code, buf_size,
                      buf_size + BUF_SIZE);
  BIB_XRETALLOC_NOSET("sv_buffer", sv_buffer, ASCII_code, buf_size,
                      buf_size + BUF_SIZE);
  BIB_XRETALLOC_NOSET("ex_buf", ex_buf, ASCII_code, buf_size,
                      buf_size + BUF_SIZE);
  BIB_XRETALLOC_NOSET("out_buf", out_buf, ASCII_code, buf_size,
                      buf_size + BUF_SIZE);
  BIB_XRETALLOC_NOSET("name_tok", name_tok, buf_pointer, buf_size,
                      buf_size + BUF_SIZE);
  BIB_XRETALLOC("name_sep_char", name_sep_char, ASCII_code, buf_size,
                buf_size + BUF_SIZE);
}

static bool input_ln(peekable_input_t *peekable) {
  last = 0; /* note: global! */
...
static void out_pool_str(rust_output_handle_t handle, str_number s) {
  pool_pointer i;
...
static void pool_overflow(void) {
  BIB_XRETALLOC("str_pool", str_pool, ASCII_code, pool_size,
                pool_size + POOL_SIZE);
}

static void out_token(rust_output_handle_t handle) {
...
static void print_bad_input_line(void) {
  buf_pointer bf_ptr;
...
static void print_skipping_whatever_remains(void) {
  puts_log("I'm skipping whatever remains of this ");
}

static void sam_wrong_file_name_print(void) {
...
static void aux_err_illegal_another_print(int32_t cmd_num) {
...
static void aux_err_no_right_brace_print(void) { puts_log("No \"}\""); }

static void aux_err_stuff_after_right_brace_print(void) {
  puts_log("Stuff after \"}\"");
}

static void aux_err_white_space_in_argument_print(void) {
  puts_log("White space in argument");
}

static void print_bib_name(void) {
...
static void check_cite_overflow(cite_number last_cite) {
  if (last_cite == max_cites) {
    BIB_XRETALLOC_NOSET("cite_list", cite_list, str_number, max_cites,
                        max_cites + MAX_CITES);
    BIB_XRETALLOC_NOSET("type_list", type_list, hash_ptr2, max_cites,
                        max_cites + MAX_CITES);
    BIB_XRETALLOC_NOSET("entry_exists", entry_exists, bool, max_cites,
                        max_cites + MAX_CITES);
    BIB_XRETALLOC("cite_info", cite_info, str_number, max_cites,
                  max_cites + MAX_CITES);
...
static void aux_end1_err_print(void) { puts_log("I found no "); }
...
static void bst_id_print(void) {
  if (scan_result == 0 /*id_null */) {
    ...
    printf_log("\"%c\" immediately follows identifier, command: ",
               buffer[buf_ptr2]);
  } else
...
static void print_missing_entry(str_number s) {
  puts_log("\nPackage BibTeX Warning: I didn't find a database entry for \"");
  print_a_pool_str(s);
  putc_log('"');
  putc_log('\n');
  mark_warning();
...
static void start_name(str_number file_name) {
  ...
  name_of_file = xmalloc_array(
      ASCII_code, (str_start[file_name + 1] - str_start[file_name]) + 1);
  name_ptr = 0;
...
static bool str_eq_buf(str_number s, buf_type buf, buf_pointer bf_ptr,
                       buf_pointer len) {
  buf_pointer i;
  pool_pointer j;
...
static bool str_eq_str(str_number s1, str_number s2) {
  if ((str_start[s1 + 1] - str_start[s1]) !=
      (str_start[s2 + 1] - str_start[s2])) {
    return false;
  }
  p_ptr1 = str_start[s1];
  p_ptr2 = str_start[s2];
...
static hash_loc str_lookup(buf_type buf, buf_pointer j, buf_pointer l,
                           str_ilk ilk, bool insert_it) {
  int32_t h;
  hash_loc p;
  buf_pointer k;
  str_number str_num;
...
static void int_to_ASCII(int32_t the_int, buf_type int_buf,
                         buf_pointer int_begin, buf_pointer *int_end) {
...
static void build_in(pds_type pds, pds_len len, hash_loc *fn_hash_loc,
                     blt_in_range blt_in_num) {
...
static bool scan3(ASCII_code char1, ASCII_code char2, ASCII_code char3) {
  buf_ptr1 = buf_ptr2;
  while ((buf_ptr2 < last) && (buffer[buf_ptr2] != char1) &&
         (buffer[buf_ptr2] != char2) && (buffer[buf_ptr2] != char3))
    buf_ptr2 = buf_ptr2 + 1;
  return buf_ptr2 < last;
}
...
static void scan_identifier(ASCII_code char1, ASCII_code char2,
                            ASCII_code char3) {
  buf_ptr1 = buf_ptr2;
  if (lex_class[buffer[buf_ptr2]] != 3 /*numeric */)
    while ((buf_ptr2 < last) &&
           (id_class[buffer[buf_ptr2]] == 1 /*legal_id_char */))
      buf_ptr2 = buf_ptr2 + 1;
  if ((buf_ptr2 - buf_ptr1) == 0)
    scan_result = 0 /*id_null */;
  else if ((lex_class[buffer[buf_ptr2]] == 1 /*white_space */) ||
           (buf_ptr2 == last))
    scan_result = 3 /*white_adjacent */;
  else if ((buffer[buf_ptr2] == char1) || (buffer[buf_ptr2] == char2) ||
           (buffer[buf_ptr2] == char3))
...
static void scan_fn_def(hash_loc fn_hash_loc) {
  ...
  while ((buffer[buf_ptr2] != 125 /*right_brace */)) {
    ...
    case 35: {
      ...
      literal_loc = str_lookup(buffer, buf_ptr1, (buf_ptr2 - buf_ptr1),
                               1 /*integer_ilk */, true);
      if (!hash_found) {
      ...
      if ((buf_ptr2 < last) &&
          (lex_class[buffer[buf_ptr2]] != 1 /*white_space */) &&
          (buffer[buf_ptr2] != 125 /*right_brace */) &&
          (buffer[buf_ptr2] != 37 /*comment */)) {
    ...
    case 39: {
      ...
      fn_loc = str_lookup(buffer, buf_ptr1, (buf_ptr2 - buf_ptr1),
                          11 /*bst_fn_ilk */, false);
      if (!hash_found) {
  ...
  }
  {
    ...
    while ((single_ptr + wiz_def_ptr > wiz_fn_space)) {
      ...
      BIB_XRETALLOC("wiz_functions", wiz_functions, hash_ptr2, wiz_fn_space,
  ...
  if (store_field) { /*257: */
    ...
    while ((buffer[buf_ptr2] != right_str_delim))
      if (buffer[buf_ptr2] == 123 /*left_brace */) {
        ...
        buf_ptr2 = buf_ptr2 + 1;
        if (!scan3(right_str_delim, 123 /*left_brace */,
                   125 /*right_brace */)) {
          if (!eat_bib_white_space()) {
            eat_bib_print();
            return false;
...
static bool scan_a_field_token_and_eat_white(void) {
  ...
  default: {
    ...
    if (store_field) {
      lower_case(buffer, buf_ptr1, (buf_ptr2 - buf_ptr1));
      macro_name_loc = str_lookup(buffer, buf_ptr1, (buf_ptr2 - buf_ptr1),
                                  13 /*macro_ilk */, false);
      store_token = true;
      if (at_bib_command) {
      ...
      if (store_token) { /*261: */
        ...
        if (ex_buf_ptr == 0) {
          ...
          if ((tmp_ptr < tmp_end_ptr) &&
              (lex_class[str_pool[tmp_ptr]] == 1 /*white_space */)) {
            {
              if (ex_buf_ptr == buf_size) {
                bib_field_too_long_print();
                return false;
...
static bool scan_and_store_the_field_value_and_eat_white(void) {
  ...
  if (store_field) { /*262: */
    ...
    if ((!at_bib_command) && (ex_buf[0] == 32 /*space */) && (ex_buf_ptr > 0))
      ...
      ex_buf_xptr = 0;
    field_val_loc = str_lookup(ex_buf, ex_buf_xptr, ex_buf_ptr - ex_buf_xptr,
                               0 /*text_ilk */, true);
    ...
    if (at_bib_command) { /*263: */
      ...
      if (field_info[field_ptr] != 0 /*missing */) {
        ...
        if ((ilk_info[field_name_loc] == crossref_num) &&
          ...
          lc_cite_loc =
              str_lookup(out_buf, ex_buf_xptr, ex_buf_ptr - ex_buf_xptr,
                         10 /*lc_cite_ilk */, true);
          if (hash_found) {
            ...
            cite_loc = str_lookup(ex_buf, ex_buf_xptr, ex_buf_ptr - ex_buf_xptr,
                                  9 /*cite_ilk */, true);
            if (hash_found)
              hash_cite_confusion();
...
static bool von_token_found(void) {
  ...
  while (name_bf_ptr < name_bf_xptr)
    if ((sv_buffer[name_bf_ptr] >= 'A') && (sv_buffer[name_bf_ptr] <= 'Z'))
      return false;
    else if ((sv_buffer[name_bf_ptr] >= 'a') &&
             (sv_buffer[name_bf_ptr] <= 'z')) {
      return true;
      ...
      if ((name_bf_ptr + 2 < name_bf_xptr) &&
          (sv_buffer[name_bf_ptr] == 92 /*backslash */)) { /*399: */
        name_bf_ptr = name_bf_ptr + 1;
        name_bf_yptr = name_bf_ptr;
        while ((name_bf_ptr < name_bf_xptr) &&
               (lex_class[sv_buffer[name_bf_ptr]] == 2 /*alpha */))
          name_bf_ptr = name_bf_ptr + 1;
        control_seq_loc =
            str_lookup(sv_buffer, name_bf_yptr, name_bf_ptr - name_bf_yptr,
                      ...
                      while (name_bf_ptr < name_bf_xptr) {
                        ...
                        if (lex_class[sv_buffer[name_bf_ptr]] == 2 /*alpha */) {
                          ...
                          goto loop_exit;
                                   ...
                                   (sv_buffer[name_bf_ptr] ==
                                    123 /*left_brace */)) {

                          if (sv_buffer[name_bf_ptr + 1] ==
                              92 /*backslash */) { /*417: */
                            ...
                            while ((name_bf_ptr < name_bf_xptr) &&
                              ...
                              if (sv_buffer[name_bf_ptr] ==
                                  125 /*right_brace */)
                                nm_brace_level = nm_brace_level - 1;
                              else if (sv_buffer[name_bf_ptr] ==
                                       123 /*left_brace */)
                                nm_brace_level = nm_brace_level + 1;
                              {
                                if (ex_buf_ptr == buf_size)
...
static void push_lit_stk(int32_t push_lt, stk_type push_type) {
  ...
  if (lit_stk_ptr == lit_stk_size) {
    BIB_XRETALLOC_NOSET("lit_stack", lit_stack, int32_t, lit_stk_size,
                        lit_stk_size + LIT_STK_SIZE);
    BIB_XRETALLOC("lit_stk_type", lit_stk_type, stk_type, lit_stk_size,
                  lit_stk_size + LIT_STK_SIZE);
  }
...
static void print_wrong_stk_lit(int32_t stk_lt, stk_type stk_tp1,
                                stk_type stk_tp2) {
...
static void add_out_pool(str_number p_str) {
  ...
  while ((out_buf_length > max_print_line) && (!unbreakable_tail)) { /*324: */
    ...
    while ((lex_class[out_buf[out_buf_ptr]] != 1 /*white_space */) &&
    ...
    if (pop_lit2 >= cmd_str_ptr) {
      ...
      if (pop_lit1 >= cmd_str_ptr) {
        ...
        if ((str_start[pop_lit1 + 1] - str_start[pop_lit1]) == 0)
          ...
          {
            while ((pool_ptr + (str_start[pop_lit1 + 1] - str_start[pop_lit1]) +
                        (str_start[pop_lit2 + 1] - str_start[pop_lit2]) >
                    pool_size))
              pool_overflow();
          }
          sp_ptr = str_start[pop_lit2];
...
static void x_gets(void) {
  ...
  if (pop_typ1 != 2 /*stk_fn */)
    print_wrong_stk_lit(pop_lit1, pop_typ1, 2 /*stk_fn */);
            ...
            ((fn_type[pop_lit1] == 6 /*str_entry_var */) ||
             (fn_type[pop_lit1] == 5 /*int_entry_var */))))
    ...
    case 5:
      ...
      if (pop_typ2 != 0 /*stk_int */)
        ...
        entry_ints[cite_ptr * num_ent_ints + ilk_info[pop_lit1]] =
            /*:356 */ pop_lit2;
      break;
    case 6: {
      if (pop_typ2 != 1 /*stk_str */)
        ...
        while (sp_ptr < sp_xptr1) {
          ...
          entry_strs[(str_ent_ptr) * (ent_str_size + 1) + (ent_chr_ptr)] =
              str_pool[sp_ptr];
      ...
      if (pop_typ2 != 1 /*stk_str */)
        ...
        if (pop_lit2 < cmd_str_ptr)
          ...
          while (sp_ptr < sp_end) {
            ...
            global_strs[(str_glb_ptr) * (glob_str_size + 1) + (glob_chr_ptr)] =
                str_pool[sp_ptr];
...
static void x_change_case(void) {
  ...
  if (pop_typ1 != 1 /*stk_str */) {
...

    {
      ...
      if (((str_start[pop_lit1 + 1] - str_start[pop_lit1]) != 1) ||
          (conversion_type == 3 /*bad_conversion */)) {
        conversion_type = 3 /*bad_conversion */;
        print_a_pool_str(pop_lit1);
          ...
          if (conversion_type == 0 /*title_lowers */) {
            ...
            if (ex_buf_ptr == 0)
              goto lab21;
                     ...
                     (lex_class[ex_buf[ex_buf_ptr - 1]] == 1 /*white_space */))
              goto lab21;
          }
          {
            ex_buf_ptr = ex_buf_ptr + 1;
...
static void x_missing(void) {
  ...
  if (!mess_with_entries)
    bst_cant_mess_with_entries_print();
  else if ((pop_typ1 != 1 /*stk_str */) &&
           (pop_typ1 != 3 /*stk_field_missing */)) {
    if (pop_typ1 != 4 /*stk_empty */) {
      print_stk_lit(pop_lit1, pop_typ1);
      puts_log(", not a string or missing field,");
      bst_ex_warn_print();
...
static void x_substring(void) {
  ...
  if (pop_typ1 != 0 /*stk_int */) {
    ...
    if ((pop_lit1 <= 0) || (pop_lit2 == 0) || (pop_lit2 > sp_length) ||
        (pop_lit2 < -(int32_t)sp_length)) {
      push_lit_stk(s_null, 1 /*stk_str */);
      return;
    } else { /*439: */
              ...
              while ((ex_buf_ptr < ex_buf_length) && (brace_level > 0)) {
                ...
                if ((ex_buf_ptr < ex_buf_length) && (ex_buf_ptr == ex_buf_xptr))
                  ...
                  if (hash_found) { /*454: */
                    ...
                    default:
                      string_width =
                          string_width + char_width[ex_buf[ex_buf_xptr]];
                      break;
                    }
                  }
                }
                ...
                while (((ex_buf_ptr < ex_buf_length) && (brace_level > 0) &&
                  ...
                  if (ex_buf[ex_buf_ptr] == 125 /*right_brace */)
                    ...
                    string_width =
                        string_width + char_width[ex_buf[ex_buf_ptr]];
                  ex_buf_ptr = ex_buf_ptr + 1;
                }
              }
              ex_buf_ptr = ex_buf_ptr - 1;
...
static void execute_fn(hash_loc ex_fn_loc) {
  ...
  case 5: {
    if (!mess_with_entries)
      ...
      push_lit_stk(entry_ints[cite_ptr * num_ent_ints + ilk_info[ex_fn_loc]],
                   0 /*stk_int */);
  } break;
  case 6: {
    if (!mess_with_entries)
      ...
      ex_buf_ptr = 0;
      while (entry_strs[(str_ent_ptr) * (ent_str_size + 1) + (ex_buf_ptr)] !=
             127 /*end_of_string */) {

        ex_buf[ex_buf_ptr] =
            entry_strs[(str_ent_ptr) * (ent_str_size + 1) + (ex_buf_ptr)];
        ex_buf_ptr = ex_buf_ptr + 1;
      }
      ex_buf_length = ex_buf_ptr;
      add_pool_buf_and_push();
...
static int get_the_top_level_aux_file_name(const char *aux_file_name) {
  ...
  aux_name_length -=
      4; /* strip off the (assumed) ".aux" for subsequent futzing */
  name_length = aux_name_length;

  /* this code used to auto-add the .aux extension if needed; we don't */

  aux_ptr = 0;
  if ((aux_file[aux_ptr] = peekable_open((char *)name_of_file, kpse_tex_format)) ==
      NULL) {
  ...
  while (name_ptr < name_length) {
    buffer[name_ptr + 1] =
        name_of_file[name_ptr]; // preserve pascal-style string semantics
    name_ptr = name_ptr + 1;
  }

  top_lev_str =
      hash_text[str_lookup(buffer, 1, aux_name_length, 0 /*text_ilk*/, true)];
  aux_list[aux_ptr] =
      hash_text[str_lookup(buffer, 1, name_length, 3 /*aux_file_ilk*/, true)];
...
static void aux_bib_data_command(void) {
  ...
  while (buffer[buf_ptr2] != 125 /*right_brace */) {
    ...
    }
    {
      ...
      bib_list[bib_ptr] = hash_text[str_lookup(
          buffer, buf_ptr1, (buf_ptr2 - buf_ptr1), 6 /*bib_file_ilk */, true)];
      if (hash_found) {
      ...
      if ((bib_file[bib_ptr] = peekable_open((char *)name_of_file, kpse_bib_format)) ==
          NULL) {
        puts_log("\nPackage BibTeX Warning: I couldn't open database file ");
        print_bib_name();
        aux_err_print();
        return;
      }
...
static void aux_bib_style_command(void) {
  ...
  {
    bst_str = hash_text[str_lookup(buffer, buf_ptr1, (buf_ptr2 - buf_ptr1),
                                   5 /*bst_file_ilk */, true)];
    if (hash_found) {
    ...
    start_name(bst_str);
    if ((bst_file = peekable_open((char *)name_of_file, kpse_bst_format)) == NULL) {
...
static void aux_citation_command(void) {
  ...
  while (buffer[buf_ptr2] != 125 /*right_brace */) {
    ...
    }
    {
      ...
      lc_cite_loc = str_lookup(ex_buf, buf_ptr1, (buf_ptr2 - buf_ptr1),
                               10 /*lc_cite_ilk */, true);
      if (hash_found) { /*136: */
        ;

        dummy_loc = str_lookup(buffer, buf_ptr1, (buf_ptr2 - buf_ptr1),
                               9 /*cite_ilk */, false);
        if (!hash_found) {
        ...
        cite_loc = str_lookup(buffer, buf_ptr1, (buf_ptr2 - buf_ptr1),
                              9 /*cite_ilk */, true);
        if (hash_found)
          hash_cite_confusion();
  ...
  }
  {
    ...
    if ((buf_ptr2 - buf_ptr1) <
      ...
      aux_extension_ok = false;
                          ...
                          buf_ptr2 - (str_start[s_aux_extension + 1] -
                                      str_start[s_aux_extension]),
                          (str_start[s_aux_extension + 1] -
                           str_start[s_aux_extension]))))
      aux_extension_ok = false;
    if (!aux_extension_ok) {
      print_a_token();
    ...
    aux_list[aux_ptr] = hash_text[str_lookup(
        buffer, buf_ptr1, (buf_ptr2 - buf_ptr1), 3 /*aux_file_ilk */, true)];
    if (hash_found) {
...
static int pop_the_aux_stack(void) {
...
static void get_aux_command_and_process(void) {
  ...
  if (!scan1(123 /*left_brace */))
    return;
  command_num = ilk_info[str_lookup(buffer, buf_ptr1, (buf_ptr2 - buf_ptr1),
...
static bool bad_argument_token(void) {
  ...
  if (!hash_found) {
    ...
    bst_err_print_and_look_for_blank_line();
    return true;
  } else if ((fn_type[fn_loc] != 0 /*built_in */) &&
             (fn_type[fn_loc] != 1 /*wiz_defined */)) {
    print_a_token();
...
static void bst_function_command(void) {
  ...
  {{if (buffer[buf_ptr2] != 123 /*left_brace */){bst_left_brace_print();
  puts_log("function");
  bst_err_print_and_look_for_blank_line();
  return;
}
...
}
{
  ...
  lower_case(buffer, buf_ptr1, (buf_ptr2 - buf_ptr1));
  wiz_loc = str_lookup(buffer, buf_ptr1, (buf_ptr2 - buf_ptr1),
                       11 /*bst_fn_ilk */, true);
  {
    if (hash_found) {
...
}
{
  ...
  }
  {
    ...
    macro_def_loc = str_lookup(buffer, buf_ptr1, (buf_ptr2 - buf_ptr1),
                               0 /*text_ilk */, true);
...
static void get_bib_command_or_entry_and_process(void) {
  ...
  }
  {
    ...
    scan_identifier(123 /*left_brace */, 40 /*left_paren */,
                    40 /*left_paren */);
    {
      if ((scan_result == 3 /*white_adjacent */) ||
          (scan_result == 1 /*specified_char_adjacent */))
    ...
    if (hash_found) { /*240: */
      ...
      case 2: {
        ...
        }
        {
          ...
          scan_identifier(61 /*equals_sign */, 61 /*equals_sign */,
                          61 /*equals_sign */);
          {
            if (((scan_result == 3 /*white_adjacent */) ||
                 (scan_result == 1 /*specified_char_adjacent */)))
          ...
          }
          {
            ...
            lower_case(buffer, buf_ptr1, (buf_ptr2 - buf_ptr1));
            cur_macro_loc = str_lookup(buffer, buf_ptr1, (buf_ptr2 - buf_ptr1),
                                       13 /*macro_ilk */, true);
      ...
      entry_type_loc = str_lookup(buffer, buf_ptr1, (buf_ptr2 - buf_ptr1),
                                  11 /*bst_fn_ilk */, false);
  ...
  }
  {
    while (buffer[buf_ptr2] != right_outer_delim) {
        ...
        goto loop_exit;
      {
        ...
        if (store_entry) {
          lower_case(buffer, buf_ptr1, (buf_ptr2 - buf_ptr1));
          field_name_loc = str_lookup(buffer, buf_ptr1, (buf_ptr2 - buf_ptr1),
                                      11 /*bst_fn_ilk */, false);
          if (hash_found) {
...
static void bst_read_command(void) {
  ...
  }
  {
...

    {
      ...
      {
        entry_strs =
            XTALLOC((num_ent_strs + 1) * (num_cites + 1) * (ent_str_size + 1),
                    ASCII_code);
        str_ent_ptr = 0;
        while (str_ent_ptr < num_ent_strs * num_cites) {

          entry_strs[(str_ent_ptr) * (ent_str_size + 1) + (0)] =
              127 /*end_of_string */;
...
static void bst_sort_command(void) {
  if (!read_seen) {
...
static void bst_strings_command(void) {
  if (!eat_bst_white_space()) {
    eat_bst_print();
    puts_log("strings");
    bst_err_print_and_look_for_blank_line();
  ...
  while (buffer[buf_ptr2] != 125 /*right_brace */) {
    ...
    if (num_glb_strs == max_glob_strs) {
      ...
      BIB_XRETALLOC("glb_str_end", glb_str_end, int32_t, max_glob_strs,
                    max_glob_strs + MAX_GLOB_STRS);
...
static void get_bst_command_and_process(void) {
  if (!scan_alpha()) {
...
static int initialize(const char *aux_file_name) {
  int32_t i;
  hash_loc k;
...
void bibmem_cleanup(){
  free(bib_file); bib_file=0;
  free(bib_list); bib_list=0;
  free(wiz_functions); wiz_functions=0;
  free(field_info); field_info= 0;
  free(s_preamble); s_preamble=0;
  free(str_pool); str_pool=0;
  free(buffer); buffer=0;
  free(sv_buffer); sv_buffer=0;
  free(ex_buf); ex_buf=0;
  free(out_buf);  out_buf=0;
  free(name_tok); name_tok=0;
  free(name_sep_char); name_sep_char=0;
  free(glb_str_ptr); glb_str_ptr=0;
  free(global_strs); global_strs=0;
  free(glb_str_end);  glb_str_end=0;
  free(cite_list);  cite_list=0;
  free(type_list); type_list=0;
  free(entry_exists); entry_exists=0;
  free(cite_info); cite_info=0;
  free(str_start); str_start=0;
  free(hash_next); hash_next=0;
  free(hash_text); hash_text=0;
  free(hash_ilk); hash_ilk=0;
  free(ilk_info); ilk_info=0;
  free(fn_type); fn_type=0;
  free(lit_stack); lit_stack=0;
  free(lit_stk_type); lit_stk_type=0;
